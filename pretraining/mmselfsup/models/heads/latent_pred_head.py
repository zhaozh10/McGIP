...
@HEADS.register_module()
class LatentGazePredictHead(BaseModule):
    """Head for latent feature prediction.
    ...
    def __init__(self, predictor: dict) -> None:
        super(LatentGazePredictHead, self).__init__()
        self.predictor = build_neck(predictor)

    def forward(self, input: torch.Tensor, target: torch.Tensor,labels:torch.Tensor) -> dict:
        """Forward head.

        Args:
            input (Tensor): NxC input features.
            target (Tensor): NxC target features.
            labels (Tensor): NxN labels indicating subsequent computation criterion
        ...
        target_norm = nn.functional.normalize(target, dim=1)
        # loss = -(pred_norm * target_norm).sum(dim=1).mean()
        loss=torch.matmul(pred_norm,target_norm.T)*labels
        # print(loss)
        loss=-torch.sum(loss)
        # print("===============")
        # print(loss)
        loss=loss/num_pair
        # logits=torch.mul(pred_norm,target_norm)
        # loss = -(pred_norm * target_norm).sum(dim=1).mean()
        return dict(loss=loss)
